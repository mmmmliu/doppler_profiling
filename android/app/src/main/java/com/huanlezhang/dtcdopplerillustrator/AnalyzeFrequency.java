...
public class AnalyzeFrequency {
    ...
    public int[] mChWindowMax = new int[N_CH_DOT];
    public int[] mChWindowDetect = new int[N_CH_DOT];
    ...
    public int mFREQ_SOUND;

    AnalyzeFrequency(Handler mainHandler, Runnable drawRun, int FREQ_SOUND) {
        mHandler = mainHandler;
        mRunable = drawRun;
        mFREQ_SOUND = FREQ_SOUND;
                    ...
                    while (remainFftBufSize > 0 && mIsRecording) {
                        ...
                        for (int i = 0; i < audioReadSize; i++) {
                            ...
                            if ((i % UNDER_SAMPLE_X) == 0) {
                                mFftBuffer[indexFftBuf++] = filter((double) mAudioBuffer[i]) / (4096 * 8);
                                remainFftBufSize--;
                            } else {
    ...
    private void calculateFrequencyOffset() {
        ...
        int rangeLeftWide = 400;

        int curIndex = (int) (2048 * ((mFREQ_SOUND - 18000) / 3000.0));
        int leftIndex = (int) (2048 * ((mFREQ_SOUND - rangeLeftWide - 18000) / 3000.0));
        int rightIndex = (int) (2048 * ((mFREQ_SOUND + rangeLeftWide - 18000) / 3000.0));
...
//        rightIndex = 1024;
        ...
        mCh[mChIndex] = (int) (FREQ_RESOLUTION * (maxIndex - curIndex)) ; //delta_f
        mCh[mChIndex] = (int) (mCh[mChIndex] * 1.0 / mFREQ_SOUND * 346 * 100); //delta_v = delta_f / f * v;

        int newWindowMax = mCh[mChIndex];
        for(int i = -1; i > -11; i--){
            int mChWindowIndex = ((mChIndex + i) + N_CH_DOT) % N_CH_DOT;
            newWindowMax = Math.max(newWindowMax, mCh[mChWindowIndex]);
        }
        mChWindowMax[mChIndex] = newWindowMax;

        mChWindowDetect[mChIndex] = 0;
        int[] res = helper(mChWindowMax, mChIndex);
        if(res[0] != -1){
            for(int i = 1; i <= res[1]; i++){
                mChWindowDetect[correctIndex(mChIndex - i, N_CH_DOT)] = res[0];
            }
        }
    ...
    private static int[] helper(int[] arr, int index){
        ...
        int len = arr.length;
        int sum = 0;
        if(arr[index] <= 0 && arr[correctIndex(index - 1, len)] > 0){
            for(int i = 1; i < len; i++){
                if(arr[correctIndex(index - i, len)] <= 0 && arr[correctIndex(index - i + 1, len)] > 0){
                    int distance = i - 1;
                    if(distance >= 5){
                        int average = sum / distance;
                        res[0] = average;
                        res[1] = distance;
                        return res;
                    }
                    break;
                }else if(arr[correctIndex(index - i, len)] < 0){
                    return res;
                }else {
                    sum += arr[correctIndex(index - i, len)];
                }
            }
            return res;
        }else{
    ...
    private static int correctIndex(int index, int len){
        while (index < 0){
            index += len;
        }
        return index % len;
